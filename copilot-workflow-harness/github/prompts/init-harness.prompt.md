---
name: init-harness
description: "Stand up (or repair) the harness for THIS app: author a correct harness.config.json and prove it with the trust scripts. The judgment half the deterministic installer can't do."
agent: agent
argument-hint: "optional app dir in a multi-app workspace (default: this workspace)"
---

Author a **correct, self-tested `harness.config.json`** for this app. The installer (`install.mjs`) already did the mechanical 80% — vendored the kit, installed the `.github/` bindings, autodetected a package manager. You do the judgment 20% that needs reading this specific codebase. You only write `harness.config.json` (and fix it in repair mode); never production code. The schema is `copilot-workflow-harness/harness.config.schema.json`; start from `harness.config.example.json` if authoring fresh, repair-in-place if one exists (never clobber a working config or an existing plan dir).
...
1. `node copilot-workflow-harness/scripts/hook-selftest.mjs` → exit 0 (hooks bite).
2. `node copilot-workflow-harness/scripts/run-gate.mjs --no-runtime` → exit 0 (every verb resolves and passes).
3. If a plan exists: `node copilot-workflow-harness/scripts/phase-items.mjs <Pid>` → exit 0 with sane JSON.
4. Verifier loads: for `web`, `playwright-core` installed and `node copilot-workflow-harness/verifiers/web.mjs` runs (a `SETUP` verdict with the dev server down still proves it resolves).

Cite each command and exit code. Not done until all are green.

## 4. Report

The profile · every non-obvious field and why (verb corrections, verifier choice, routes enumerated) · the self-test evidence · what needs the human (auth creds, any verifier gap knowingly left at `"none"`) · next command: `/plan-work-item <brief>`, or `/run-phase <id>` if a plan exists.

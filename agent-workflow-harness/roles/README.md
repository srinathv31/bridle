...
you. Run them as `node agent-workflow-harness/scripts/<name>.mjs`.
...
- **Work-items within a phase run in parallel only when their file sets don't overlap**, sequential
  ...
  `node agent-workflow-harness/scripts/phase-items.mjs <Pid>`, which reads each item's _Files this item creates /
...
- **Phases are merge barriers.** You do not advance past a phase until it is complete _and QA-passed_
  — proven by `node agent-workflow-harness/scripts/phase-guard.mjs <Pid>` exiting 0.
...
| ------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `node agent-workflow-harness/scripts/phase-items.mjs <Pid>`                 | Deterministic plan reader: emits each item's file set and computes parallel-safety from **real file-set overlap**, overriding any brief's self-reported claim.        |
| `node agent-workflow-harness/scripts/precheck.mjs <id>`                     | Legal-dispatch guard, run **before** spawning: refuses a missing brief, an already-`[x]` item, an unmet `Depends on:`, or a phase with incomplete/unguarded upstream. |
| `node agent-workflow-harness/scripts/run-gate.mjs [--build] [--no-runtime]` | Whole-repo detector gate: runs the configured `lint` / `typecheck` / `test` verbs (+ `build` with `--build`) then the **runtime verifier**. One structured verdict.   |
| `node agent-workflow-harness/scripts/qa-check.mjs <report>`                 | Mechanical evidence check on a QA report: cited screenshots / `file:line` must resolve; file-attributed quotes must appear verbatim in the file.                      |
| `node agent-workflow-harness/scripts/phase-guard.mjs <Pid>`                 | Merge barrier: a phase is done only when **every item is `[x]`** AND a phase QA report exists that itself passes `qa-check`.                                          |
| `node agent-workflow-harness/scripts/subagent-status.mjs`                   | Heartbeat reader: flags a `running` subagent with a stale `lastBeat` as STUCK; carries handoff state.                                                                 |
| `node agent-workflow-harness/scripts/hook-selftest.mjs`                     | Feeds the configured lint/format hooks a known-bad fixture; fails if a hook is dead or a no-op.                                                                       |
...
- **a path** — your own adapter (e.g. an `"api"` verifier that asserts an endpoint answers with a
  ...
  live in `agent-workflow-harness/verifiers/`; see `agent-workflow-harness/verifiers/README.md`.
...
three live in `agent-workflow-harness/templates/`.)
...
- `node agent-workflow-harness/scripts/precheck.mjs <Pid>` **before** launching a phase. Red precheck = do not
  launch.
- Watch the run via heartbeats: poll `node agent-workflow-harness/scripts/subagent-status.mjs` on an interval. A
...
- Run `node agent-workflow-harness/scripts/hook-selftest.mjs` once at session start — a dead hook means no inline
  protection.
...
- Every claim in your report cites ground truth. Quote files **verbatim** with `path:line`. Before
  handing off, run `node agent-workflow-harness/scripts/qa-check.mjs <your-report>` — a non-zero exit means a

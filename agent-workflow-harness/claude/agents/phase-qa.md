...
Follow the `phase-qa` skill at `.claude/skills/phase-qa/SKILL.md` end-to-end. The skill will auto-detect thorough mode (no prior pass file exists). Run the full battery.
...
- **All five QA legs are required:** static checks (typecheck + build, **not** lint), the runtime verifier (`harness.config.json` → `runtime.verifier`), runtime checks via Playwright MCP, integration seam probes, visual diff against design source. Do not skip — each catches a different bug class.
